   ...
   with col1:
        ...
        if df3.empty:
            ...
            for ind in df3.index:
                df3['TXN_AMOUNT'][ind] = currency + "" + numerize.numerize(float(df3['TXN_AMOUNT'][ind]))
            d = alt.Chart(df3).mark_arc(innerRadius=50).encode(theta=alt.Theta(field="AMOUNT", type="quantitative"),
                                                      color=alt.Color(field="TRANSACTION_TYPE", type="nominal"), tooltip=['TRANSACTION_TYPE','TXN_AMOUNT','TXN_COUNT'])
            st.altair_chart(d)

   with col2:
        st.subheader("Transactions Count")
        df_table3 = test_session.sql(
            "SELECT  COUNT(TYPE_TRAN) AS TXN_COUNT, DAY(TRANS_DATE::DATE) AS TRANSACTION_DATE,DAY(TRANS_DATE::DATE)||'/'||MONTH(TRANS_DATE::DATE) AS TXN_DATE from D_FIN_STG.FIN_STG.TRANS_DTA where (TYPE_TRAN= '" + payment_type_select + "' or '" + payment_type_select + "' ='All') and (SENDER= '" + sender_select + "' or '" + sender_select + "' ='All') and (RECEIVER= '" + receiver_select + "' or '"+ receiver_select +"' ='All') and (BANK= '" + bank_select + "' or '"+ bank_select +"' ='All') and   TRANS_DATE::DATE between '" + str(
        ...
        if df.empty:
            ...
            d = alt.Chart(df).mark_rect().encode(
                alt.X('TRANSACTION_DATE:Q', bin=alt.Bin(maxbins=30)),
                alt.Y('TXN_COUNT:Q', bin=alt.Bin(maxbins=30)),
                alt.Color('TXN_COUNT:Q', scale=alt.Scale(scheme='greenblue')),tooltip=['TXN_DATE','TXN_COUNT']
                    )
            st.altair_chart(d)
...
with st.container():
    ...
    if df.empty:
        ...
        d = alt.Chart(df).mark_bar().encode(x='TOTAL_AMOUNT:Q',y="SENDER:O", tooltip=['SENDER', 'TXN_AMOUNT'], color=alt.value('#3498db')).properties(height=300, width=1100)
        st.altair_chart(d)

with st.container():
    st.subheader("Top 5 Receivers")
    df_table3 = test_session.sql(
            "select Top 5 RECEIVER,TOTAL_AMOUNT, TOTAL_AMOUNT AS TXN_AMOUNT from(select RECEIVER,TYPE_TRAN,SUM(AMOUNT)  as TOTAL_AMOUNT from D_FIN_STG.FIN_STG.TRANS_DTA where AMOUNT IS NOT NULL and (TYPE_TRAN= '" + payment_type_select + "' or '" + payment_type_select + "' ='All') and (SENDER= '" + sender_select + "' or '" + sender_select + "' ='All') and (RECEIVER= '" + receiver_select + "' or '"+ receiver_select +"' ='All') and (BANK= '" + bank_select + "' or '"+ bank_select +"' ='All') and  TRANS_DATE::DATE between '" + str(
                from_dt) + "' and '" + str(
                to_dt) + "' group by 1,2 order by Total_Amount desc)order by Total_Amount desc").collect()
    df = pd.DataFrame(df_table3)
...
with st.container():
    ...
    df_table3 = test_session.sql(
            "select BANK,TYPE_TRAN AS TRANSACTION_TYPE,COUNT(TYPE_TRAN) AS TXN_COUNT,SUM(AMOUNT) as TOTAL_AMOUNT,SUM(AMOUNT) as TXN_AMOUNT from D_FIN_STG.FIN_STG.TRANS_DTA  where BANK in(Select BANK from(select Top 5 BANK,TYPE_TRAN,TOTAL_AMOUNT from(select BANK,TYPE_TRAN,SUM(AMOUNT)  as TOTAL_AMOUNT from D_FIN_STG.FIN_STG.TRANS_DTA where (TYPE_TRAN= '" + payment_type_select + "' or '" + payment_type_select + "' ='All') and (SENDER= '" + sender_select + "' or '" + sender_select + "' ='All') and (RECEIVER= '" + receiver_select + "' or '"+ receiver_select +"' ='All') and (BANK= '" + bank_select + "' or '"+ bank_select +"' ='All') and  TRANS_DATE::DATE between '" + str(
                from_dt) + "' and '" + str(
    ...
    if df.empty:
        ...
        d = alt.Chart(df).mark_bar().encode(
                x='BANK',
                y='TXN_COUNT',
                color='TRANSACTION_TYPE',tooltip=['BANK', 'TXN_AMOUNT','TRANSACTION_TYPE','TXN_COUNT'],
...
with st.container():
    ...
    if df.empty:
        ...
        y='TOTAL_AMOUNT:Q',
        color='YEAR:N',
        column='BANK:N', tooltip=['YEAR', 'BANK','TXN_AMOUNT']
