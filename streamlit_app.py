...
with st.container():
   ...
   with col1:
        ...
        if df3.empty:
            ...
            ).properties(height=450, width=725)
            st.altair_chart(d)
...
with st.container():
    ...
    with col1:
        ...
        if df.empty:
            ...
            d = alt.Chart(df).mark_bar().encode(x='TOTAL_AMOUNT:Q',y="SENDER:O", tooltip=['SENDER', 'TOTAL_AMOUNT'], color=alt.value('#3498db')).properties(height=450, width=725)
            st.altair_chart(d)
    ...
    with col2:
        ...
        if df.empty:
            ...
            d = alt.Chart(df).mark_bar().encode(x='BANK',y='TOTAL_AMOUNT',tooltip=['BANK', 'TOTAL_AMOUNT'], color=alt.value('#3498db')).properties(height=450, width=725)
            st.altair_chart(d)
