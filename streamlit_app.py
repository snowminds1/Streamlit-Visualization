...
with st.container():
   ...
   with col2:
        ...
        if df.empty:
            ...
            #st.altair_chart(d)
            d=alt.Chart(df).mark_arc(innerRadius=35).encode(theta=alt.Theta(field="TOTAL_AMOUNT", type="quantitative"),color=alt.Color(field="RECEIVER", type="nominal"),tooltip=['RECEIVER','TOTAL_AMOUNT'],).properties(height=300, width=500)
            st.altair_chart(d)
...
with st.container():
    ...
    with col1:
        ...
        if df.empty:
            ...
            d = alt.Chart(df).mark_bar().encode(x='TOTAL_AMOUNT:Q',y="SENDER:O", tooltip=['SENDER', 'TOTAL_AMOUNT'], color=alt.value('#3498db')).properties(height=300, width=500)
            st.altair_chart(d)
    ...
    #         st.write("No Data Found")
    #     else:
    #         d = alt.Chart(df).mark_bar().encode(x='BANK',y='TOTAL_AMOUNT',tooltip=['BANK', 'TOTAL_AMOUNT'], color=alt.value('#3498db')).properties(height=300, width=500)
    #         st.altair_chart(d)
